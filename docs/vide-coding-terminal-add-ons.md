...
**Usage:**
```bash
backstop init          # Initialize config in current directory
backstop reference     # Capture reference screenshots
backstop test          # Run visual regression test against references
backstop approve       # Approve latest test results as new reference
```

**What it does:**
- Captures screenshots of pages/components at defined viewports
- Diffs new screenshots against saved references pixel-by-pixel
- Generates an HTML report showing pass/fail with visual diffs
- Useful for catching unintended CSS or layout regressions before deploying

**Config file:** `backstop.json` (generated by `backstop init`)
...
- **BackstopJS + dependencies:** ~150 MB
